const addSuffix = date => {
    ...
    const lastChr = dateString.charAt(dateString.length -1);

    if(lastChr === '1' && dateString !== '11')
    {
        dateString = `${dateString}st`;
    }
    else if(lastChr === '2' && dateString !== '12')
    {
        dateString = `${dateString}nd`;
    }
    else if(lastChr === '3' && dateString !== '13')
    {
...
module.exports =(timestamp, {monthLength = 'short', dateSuffix = true}={})=>{
    ...
    const months = {
      ...
      const dayOfMonth = dateSuffix?addSuffix(dateObj.getDate()):dateObj.getDate();
